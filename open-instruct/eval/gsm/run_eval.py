...
import vllm
import evaluate
from eval.utils import (generate_completions, load_hf_lm,
                        query_openai_chat_model, dynamic_import_function,
                        load_hf_tokenizer, upload_results_to_hf,
...
def main(args):
        ...
        for example in GSM_EXAMPLARS:
            if args.no_cot:
                ...
                demonstrations.append("Question: " + example["question"] +
                                      "\n" + "Answer: " +
                                      example["cot_answer"])
        prompt_prefix = "Answer the following questions. Let's think step by step.\n\n" + "\n\n".join(
            demonstrations) + "\n\n"
    else:
    ...
    if args.model_name_or_path:
        ...
        if args.use_vllm:
            ...
            if args.use_chat_format:
                prompts = [
                    apply_chat_format(example, tokenizer)
                    for example in test_data
                ]
            else:
                prompts = [
            ...
            if isinstance(model, GPTNeoXForCausalLM) or isinstance(
                ...
                print(
                    "Set tokenizer.model_max_length to model.config.max_position_embeddings: {}"
            ...
            stop_tokens += [[
                tokenizer.encode(stop_seq, add_special_tokens=False)[-1]
            ] for stop_seq in args.additional_stop_sequence]
            if args.stop_at_double_newline:
                ...
                if new_line_token == double_new_line_token:
                    stop_tokens = [new_line_token, new_line_token
                                   ]  # double new line is two new line tokens
                else:
                    stop_tokens = [double_new_line_token
                                   ]  # double new line has its own token
            outputs = generate_completions(
                ...
                None,  # we only use stop token for non-chat format (usually applied to vanilla pretrained language models). For chat format, we will rely on the model knows when to stop.
                do_sample=False,
            )
    else:
        instances = [{
    ...
    for output in outputs:
        ...
        match = re.search(r"So the answer is(.*?)\.", output)
        if match:
    ...
    if args.upload_to_hf is not None:
        ...
        check_and_upload_model_metadata(args.model_name_or_path,
                                        args.upload_to_hf,
                                        args.hf_upload_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_num_examples",
                        type=int,
                        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "if specified, we will load the model from a revision of the model in the hub"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        "if specified, we will use the OpenAI API to generate the predictions."
    )
    parser.add_argument(
        "--n_shot",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "load model in 8bit mode, which will reduce memory and speed up inference."
    )
    parser.add_argument(
        "--gptq",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "If given, we will use the vllm library, which will likely increase the inference throughput."
    )
    parser.add_argument(
        "--use_chat_format",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help=
        "The function to use to create the chat format. This function will be dynamically imported. Please see examples in `eval/templates.py`."
    )
    parser.add_argument(
        "--stop_at_double_newline",
        action="store_true",
        help=
        "If given, will stop generation at double newline instead of single.")
    parser.add_argument(
        ...
        default=[],
        help=
        "Additional stop sequences to use when generating completions. Useful for e.g. llama-3-instruct."
    )
    parser.add_argument(
        "--upload_to_hf",
        type=str,
        default=None,
        help=
        "If specified, we will upload the results to Hugging Face Datasets. "
        "This should be the name of the dataset to upload to.")
    parser.add_argument("--hf_upload_name",
                        type=str,
                        default=None,
